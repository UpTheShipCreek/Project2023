...
def tensor_get_nearest(dataIni, dataEnc, k):
    ...
    # It finds the batch/5 nearest neighbors in the original dataset and uses the indices to find their equivalent distances on the reduced dataset
...
def custom_loss_function(y_true, y_pred, k):
  ...
  def train_step(self, data):
      ...
      with tf.GradientTape() as tape:
          ...
          def calculate_loss(pair):
              ...
              loss = custom_loss_function(y_true, y_pred, k)
...
def build_autoencoder(hp):
    ...
    autoencoder.compile(optimizer='adam', loss = lambda y_true, y_pred: custom_loss_function(y_true, y_pred, batch_size // nnfactor))
    return autoencoder
...
tuner = RandomSearch(
    ...
    project_name='my_autoencoder'
)

tuner.search(train_dataset, train_dataset, epochs=epochs, batch_size=batch_size, validation_data=(val_dataset, val_dataset))
best_model = tuner.get_best_models(num_models=1)[0]

best_model.save_weights('.\python\best_encoder.keras')