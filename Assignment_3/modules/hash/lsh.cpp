...
LSH::LSH(int l, int k, double window, int tableSize, Metric* metric, int dimensions){
    this->L = l;
    this->K = k;
    this->W = window;
    this->Lmetric = metric;
    ...
    for (int i = 0; i < L; i++){
        std::shared_ptr<HashFunction> hashFunction = std::make_shared<gFunction>(this->K, this->W, this->DataDimensions);
