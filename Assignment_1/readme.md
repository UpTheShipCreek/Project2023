...
- **kMeans** is a little more intense. Of course it holds the vital infomration for the method, such as a **vector of clusters**, the **dataset** itself and the **number of clusters**. 
    ...
    - **MacQueen with Lloyd's** implements the traditional Lloyd's algorith, which calculates the distances of all the points from the **centroids**, using the **MacQueen** technique, which in turn updates each **centroid** after it gets a new assigment. **kMeans** is pressumed converged if the centroids do not change position significantly (currently specified at 1% of roughly the max distance between two points in the dataset)
    - **MacQueen with Reverse** implements **kMeans** with reverse assigment using an **approximate range search**.
    In the start it calculates the minimum distance between two clusters in order to initialize the radius (we were told to initialize the ```radius = 1/2 mindistance``` but we decided to increase it to ```radius = mindistance``` cause it yielded better results in our case). For each **centroid** we do a **range search** (using either **LSH** or **Hypercube**) with the specified radius and then for each point that falls inside the radius, we keep track of all the **clusters/centroids** that have claims on it. Then we iterate through all those claimed **points/images**, resolve the conflicts and finally assign them, double the size of the radius and try again. We stop when the clusters are gaining no new points. Finally we brute force assign all the **points/images** left without a cluster.